  ...
  port.on('message', ({ data }) => {
    ...
    void (async () => {
      try {
        ...
        const payload = data.body ? Buffer.from(data.body) : undefined;
        ...
        const response = await app.inject({ method: data.method as 'GET' | 'HEAD' | 'POST' | 'PUT' | 'PATCH' | 'DELETE' | 'OPTIONS', url: data.url, headers: { ...data.headers, host: 'app' }, payload });
