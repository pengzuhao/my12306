...
/**
 * result 字段下标（2026-09-21 对照 12306 原始报文逐字段核实）。
 *
 * 以「南京→上海」查询为例，某行字段为：
 *   [3]G4095 [4]ZAF(郑州东) [5]AOH(上海虹桥) [6]NKH(南京南) [7]AOH(上海虹桥)
 *   [8]00:01 [9]01:08 [10]01:07
 * 含义：G4095 全程为 郑州东→上海虹桥（[4]/[5]，车的终点），
 * 本段在 **南京南 00:01 上车、上海虹桥 01:08 下车**（[6]/[7]，人的上/下车站）。
 *
 * 注意：传城市级站名（如"南京"）时 12306 会按城市展开，[6]/[7] 是实际乘降站
 * （可能显示南京南而非南京），比查询入参更准，因此乘降站一律取 [6]/[7]。
 *
 * 历史问题：早期 fromStation/toStation 取 5/6，实际是「车的终点站/上车站」，
 * 方向整段反了。下单流程只用 trainCode/secretStr/时间，未受影响；本次新增
 * 车次选择器是首个真正消费乘降站的地方，故修正。
 */
export const FIELD_INDEX = {
  ...
  trainOrigin: 4, // 车次全程起点站电报码（如 郑州东）
  trainTerminal: 5, // 车次全程终点站电报码（如 上海虹桥）
  fromStation: 6, // 实际上车站（查询城市展开后的具体站，如 南京南）
  toStation: 7, // 实际下车站（如 上海虹桥）
  departTime: 8,
  arriveTime: 9,
  duration: 10,
  isCanBuy: 11, // Y/N
