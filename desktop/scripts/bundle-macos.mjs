...
const installer = path.resolve(process.argv[2] || path.join(desktop, 'release', `my12306-${version}-mac-${process.arch}-unsigned.dmg`));
...
const output = path.join(desktop, 'release', name + '.zip');
const temporary = mkdtempSync(path.join(desktop, 'release', '.bundle-'));
try {
  ...
  writeFileSync(path.join(stage, '使用说明.txt'), readFileSync(path.join(desktop, 'distribution/使用说明-macOS.txt'), 'utf8').replace('__ARCH_LABEL__', architecture));
